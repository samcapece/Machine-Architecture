...
/*
 * test_function - Test a function.  Return number of 1 if timeout occurred, 0 otherwise
 */
static int test_function(test_ptr t) {
    ...
    if (args == 0) {
        ...
        if (message_tag == TIMEOUT_TAG) {
            // Give up, function has timed out and failed
            return 1;
        ...
        for (a1 = 0; a1 < test_counts[0]; a1++) {
            ...
            if (message_tag == TIMEOUT_TAG) {
                // Give up, function has timed out
                return 1;
            } else if (message_tag != FUNC_RETURN_TAG) {
                fprintf(stderr, "Did not receive valid FUNC_RETURN_TAG from driver\n");
                exit(1);
